...
PR #43 / `NC-CLOSE-01A` ADR decisions 现为 **`MERGED_AND_VERIFIED`**。决策正文见 [NC-CLOSE-01A Runtime & State Boundaries](./plans/phase-a/nc-close-01a/nc-close-01a-runtime-state-boundaries.md)。`NC-CLOSE-01A` implementation 仍为 `NOT_AUTHORIZED`。PR #44 / `NC-CLOSE-02` Shared Contract Bindings 现为 **`MERGED_AND_VERIFIED`**（见 [bindings README](../../contracts/v1/bindings/README.md)）。PR #45 / `NC-CLOSE-03` CI MVP 现为 **`MERGED_AND_VERIFIED`**：只保护当前非临床 Freeze Candidate 工程基线，不构成临床安全、Runtime 批准、Phase A Frozen 或生产授权。PR #47 / `NC-CLOSE-01C` ADR decisions 现为 **`MERGED_AND_VERIFIED`**（见 [NC-CLOSE-01C Platform & Observability Boundaries](./plans/phase-a/nc-close-01c/nc-close-01c-platform-observability-boundaries.md)）。`NC-CLOSE-01C` implementation 仍为 `NOT_AUTHORIZED`。PR #48 / `NC-CLOSE-04` 证据现为 **`MERGED_AND_VERIFIED`**；控制状态仍为 **`PARTIALLY_VALIDATED`**（见 [NC-CLOSE-04 verification report](./evidence/phase-a/nc-close-04/nc-close-04-verification-report.md)）。不得写成 `NC-CLOSE-04` `VERIFIED` / `COMPLETE`。PR #49 / `NC-CLOSE-05` 证据现为 **`MERGED_AND_VERIFIED`**；控制状态为 **`RUNTIME_EVIDENCE_ACCOUNTING_COMPLETE`**（见 [NC-CLOSE-05 coordination report](./evidence/phase-a/nc-close-05/nc-close-05-coordination-report.md)）。`ACCOUNTING_COMPLETE != ALL_RUNTIME_PASS`。本 Draft 分支上的 `NC-CLOSE-01B` ADR decisions 为 **`PROPOSED_DECIDED_PENDING_REVIEW`**（见 [NC-CLOSE-01B Data & Retrieval Boundaries](./plans/phase-a/nc-close-01b/nc-close-01b-data-retrieval-boundaries.md)）。`NC-CLOSE-01B` implementation 仍为 `NOT_AUTHORIZED`。`TEST PASS != REQUIREMENT PASS`。`NC-CLOSE-01` umbrella implementation 与 `NC-CLOSE-06` 仍为 `NOT_AUTHORIZED`。内部排序与决策深度见 [NC-CLOSE-01 ADR scope refinement](./plans/phase-a/nc-close-01-adr-scope-refinement.md)；lane 范围仍以 [NC Closure scope register](./plans/phase-a/phase-a-non-clinical-closure-scope-register.csv) 为准。A6.5 legacy governance 已关闭；不得因此写成 A6/A7 COMPLETE，不得启动 A7-CL，不得执行 B04 提取或 C02 拒收资产映射，也不得把未来新临床内容塞回 A6.5。历史 Phase A 证据、历史 addendum、PR #29 / P7 与 dated reconciliations 分别维护。
