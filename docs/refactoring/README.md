...
PR #43 / `NC-CLOSE-01A` ADR decisions 现为 **`MERGED_AND_VERIFIED`**。决策正文见 [NC-CLOSE-01A Runtime & State Boundaries](./plans/phase-a/nc-close-01a/nc-close-01a-runtime-state-boundaries.md)。`NC-CLOSE-01A` implementation 仍为 `NOT_AUTHORIZED`。PR #44 / `NC-CLOSE-02` Shared Contract Bindings 现为 **`MERGED_AND_VERIFIED`**（见 [bindings README](../../contracts/v1/bindings/README.md)）。PR #45 / `NC-CLOSE-03` CI MVP 现为 **`MERGED_AND_VERIFIED`**：只保护当前非临床 Freeze Candidate 工程基线，不构成临床安全、Runtime 批准、Phase A Frozen 或生产授权。PR #47 / `NC-CLOSE-01C` ADR decisions 现为 **`MERGED_AND_VERIFIED`**（见 [NC-CLOSE-01C Platform & Observability Boundaries](./plans/phase-a/nc-close-01c/nc-close-01c-platform-observability-boundaries.md)）。`NC-CLOSE-01C` implementation 仍为 `NOT_AUTHORIZED`。本 Draft 分支上的 `NC-CLOSE-04` 为 **`VERIFICATION_EXECUTED_WITH_BLOCKING_FINDINGS_PENDING_REVIEW`**（见 [NC-CLOSE-04 verification report](./evidence/phase-a/nc-close-04/nc-close-04-verification-report.md)）。在 Independent Review / Merge Review / 显式合并授权完成前，不得写成 `NC-CLOSE-04` `MERGED_AND_VERIFIED` 或 `COMPLETE`。`TEST PASS != REQUIREMENT PASS`。`NC-CLOSE-01` umbrella implementation 仍为 `NOT_AUTHORIZED`；`01B` 仍为 `NOT_AUTHORIZED`。内部排序与决策深度见 [NC-CLOSE-01 ADR scope refinement](./plans/phase-a/nc-close-01-adr-scope-refinement.md)；lane 范围仍以 [NC Closure scope register](./plans/phase-a/phase-a-non-clinical-closure-scope-register.csv) 为准。A6.5 legacy governance 已关闭；不得因此写成 A6/A7 COMPLETE，不得启动 A7-CL，不得执行 B04 提取或 C02 拒收资产映射，也不得把未来新临床内容塞回 A6.5。历史 Phase A 证据、历史 addendum、PR #29 / P7 与 dated reconciliations 分别维护。
