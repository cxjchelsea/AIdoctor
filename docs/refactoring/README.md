...
PR #43 / `NC-CLOSE-01A` ADR decisions 现为 **`MERGED_AND_VERIFIED`**。决策正文见 [NC-CLOSE-01A Runtime & State Boundaries](./plans/phase-a/nc-close-01a/nc-close-01a-runtime-state-boundaries.md)。`NC-CLOSE-01A` implementation 仍为 `NOT_AUTHORIZED`。PR #50 / `NC-CLOSE-01B` ADR decisions 现为 **`MERGED_AND_VERIFIED`**（见 [NC-CLOSE-01B Data & Retrieval Boundaries](./plans/phase-a/nc-close-01b/nc-close-01b-data-retrieval-boundaries.md)）。`NC-CLOSE-01B` implementation 仍为 `NOT_AUTHORIZED`。`NC-CLOSE-01` ADR decision coverage 现为 **`COMPLETE`**；umbrella implementation 仍为 `NOT_AUTHORIZED`。PR #44 / `NC-CLOSE-02` Shared Contract Bindings 现为 **`MERGED_AND_VERIFIED`**（见 [bindings README](../../contracts/v1/bindings/README.md)）。PR #45 / `NC-CLOSE-03` CI MVP 现为 **`MERGED_AND_VERIFIED`**：只保护当前非临床 Freeze Candidate 工程基线，不构成临床安全、Runtime 批准、Phase A Frozen 或生产授权。PR #47 / `NC-CLOSE-01C` ADR decisions 现为 **`MERGED_AND_VERIFIED`**（见 [NC-CLOSE-01C Platform & Observability Boundaries](./plans/phase-a/nc-close-01c/nc-close-01c-platform-observability-boundaries.md)）。`NC-CLOSE-01C` implementation 仍为 `NOT_AUTHORIZED`。PR #48 / `NC-CLOSE-04` 证据现为 **`MERGED_AND_VERIFIED`**；控制状态仍为 **`PARTIALLY_VALIDATED`**（见 [NC-CLOSE-04 verification report](./evidence/phase-a/nc-close-04/nc-close-04-verification-report.md)）。不得写成 `NC-CLOSE-04` `VERIFIED` / `COMPLETE` / `SATISFIES_EXIT`。PR #49 / `NC-CLOSE-05` 证据现为 **`MERGED_AND_VERIFIED`**；控制状态为 **`RUNTIME_EVIDENCE_ACCOUNTING_COMPLETE`**（见 [NC-CLOSE-05 coordination report](./evidence/phase-a/nc-close-05/nc-close-05-coordination-report.md)）。`ACCOUNTING_COMPLETE != ALL_RUNTIME_PASS`。PR #51 / `NC-CLOSE-06` 现为 **`INTEGRATION_ARTIFACTS_MERGED_AND_VERIFIED`**（见 [E2E design](./plans/phase-a/nc-close-06/nc-close-06-e2e-design.md) 与 [current-state reconciliation](./evidence/phase-a/nc-close-06/nc-close-06-current-state-reconciliation.md)）。PR #52 / `NC-CLOSE-07` 现为 **`COMPLETE`**；`PHASE_A_NC_CLOSURE` 为 **`COMPLETE`**；`PHASE_A_NC_CLOSURE_EXIT` 为 **`PASS`**（见 [NC-CLOSE-07 Exit Record](./evidence/phase-a/nc-close-07/nc-close-07-exit-review-2026-08-18.md)）。A7-NC 为 **`COMPLETE`**。A7-CL 仍为 `BLOCKED`。FB-11 仍为 `UNSATISFIED`。A7 仍为 `NOT_COMPLETE`。A11 仍为 `NOT_PASSED`。PR #53 / `A7-CL-01` 现为 **`GOVERNANCE_AMENDMENT_MERGED_AND_VERIFIED`**（见 [A7-CL governance charter](./plans/phase-a/a7-cl/a7-cl-clinical-governance-charter.md) 与 [Rejoin/Exit matrix](./plans/phase-a/a7-cl/a7-cl-rejoin-exit-matrix.csv)）。本 Draft 上的 `RESCOPE-01` 为 **`ENGINEERING_BASELINE_SCOPE_AMENDMENT_AUTHORED_PENDING_REVIEW`**（见 [Engineering Baseline scope amendment](./plans/phase-a/engineering-baseline/phase-a-engineering-baseline-scope-amendment.md) 与 [gate register](./plans/phase-a/engineering-baseline/phase-a-engineering-baseline-gate-register.csv)）。Clinical Track 在本分支上为 **`PROPOSED_DEFERRED_OUT_OF_CURRENT_PRODUCT_BASELINE_PENDING_REVIEW`**。`PHASE_A_ENGINEERING_BASELINE` 为 **`SCOPE_AUTHORED_PENDING_REVIEW` / `NOT_FROZEN`**。A7-CL 在 durable Enterprise 上仍为 `BLOCKED`，直至本修订合并。不得写成 `A7-CL STARTED` / `A7-CL-02 AUTHORIZED` / `FB-11 SATISFIED` / `A7 COMPLETE` / Full A7 Exit `PASS` / A11 `PASS` / 原 Frozen Baseline `PASS` / 工程基线已冻结。Clinical Runtime 仍为 `NOT_ENABLED`；Production 仍为 `BLOCKED`；Phase B 仍为 `NOT_AUTHORIZED`。`TEST PASS != REQUIREMENT PASS`。内部排序与决策深度见 [NC-CLOSE-01 ADR scope refinement](./plans/phase-a/nc-close-01-adr-scope-refinement.md)；lane 范围仍以 [NC Closure scope register](./plans/phase-a/phase-a-non-clinical-closure-scope-register.csv) 为准。A6.5 legacy governance 已关闭；不得因此写成 A6/A7 COMPLETE，不得启动 A7-CL-02 临床内容，不得执行 B04 提取或 C02 拒收资产映射，也不得把未来新临床内容塞回 A6.5。历史 Phase A 证据、历史 addendum、PR #29 / P7 与 dated reconciliations 分别维护。
