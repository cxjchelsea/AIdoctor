...
- `current-system-inventory.md`：增加 Legacy Design Asset Inventory 维度；
- `migration-matrix.md`：增加设计原则、协议、状态、Trace、评估和知识治理资产的迁移决定；
- `coverage-matrix.md`：要求每个保留资产绑定 Owner、Contract、Phase、Test 和 Gate；
- `implementation-roadmap.md`：增加 Phase A 的 Legacy Design Asset Validation；
- `engineering-release-and-decommission-plan.md`：增加旧设计资产替代和下线门禁。

若旧文档与当前冻结基线冲突，以当前冻结基线、本文件和对应 v2.6 详细设计为准；旧文档只作为资产证据和历史设计来源。
...
`DESIGN_CONFIRMED` 只能证明设计存在，不能直接证明实现可用。最终 `KEEP` 或生产启用必须至少达到相应的代码、测试、运行或数据证据等级。
...
| 决定 | 含义 |
|---|---|
| KEEP | 职责、边界和契约基本符合目标，可直接保留 |
| ADAPT | 核心思想或逻辑有价值，但接口、权限、状态、版本或依赖必须调整 |
| EXTRACT | 从旧代码、配置或文档中抽取为独立治理资产 |
| SPLIT | 一个旧资产跨越多个目标模块，需要拆分 |
| MERGE | 多个旧资产职责重叠，需要合并为单一目标能力 |
| WRAP | 通过 Legacy Adapter 暂时接入，待新实现稳定后迁移 |
| EVALUATE | 可能有价值，但必须通过对照实验、来源治理或安全评估 |
| ARCHIVE | 不进入当前生产主链路，但保留历史、研究或参考价值 |
| REWRITE | 目标职责需要保留，但当前实现不满足状态、安全或工程边界 |
| REMOVE | 已完成替代、迁移、归档和回滚窗口后才允许删除 |

禁止仅因为目录、服务名或旧文档不符合目标架构就直接 `REMOVE`。
...
|---|---|---|---|---|---|
| 双通道推理 | KEEP + ADAPT | 结构化推理与语言表达分离 | Clinical Intelligence、Safety、Evidence、Model Runtime | LLM 不拥有最终临床决策权 | A-D |
| 单主 Agent / 单一最终提交者 | ADAPT | 统一编排和责任链 | Agent Runtime + State Committer + Safety | 拆分规划、提交和安全权限 | B-D |
| 无状态工具 | KEEP + ADAPT | 工具无长期目标、返回结构化结果 | Tool/Skill Governance | 限制 Context、版本、权限和副作用 | A-D |
...
| CDP 唯一临床聚合视图 | KEEP + SPLIT | 统一病例视图和版本 | Clinical State & Data Foundation | 大 JSON 拆为结构化实体和 Ledger | A-D |
| CDP Copy-on-Write | KEEP + ADAPT | 版本、历史和回放 | State Committer + Version Ledger | 乐观并发、幂等、不可变事件 | B |
| AgentState | ADAPT | 预算、已尝试工具、回退和停止条件 | GraphState + Run/Checkpoint | PostgreSQL Durable Store 为事实源 | A-C |
| AuditTrail | KEEP + SPLIT | 追加写、工具/状态/决策可追溯 | Technical Trace、AgentEvent、Decision、Compliance Audit | PHI、版本链和职责拆分 | B-F |
| `@TraceExecution` | KEEP + ADAPT | 领域语义注解、低侵入埋点 | Observability Instrumentation | 对接 OTel Span，不隐藏临床逻辑 | A-C |
...
| Feign Trace Interceptor | ADAPT | 跨服务上下文传播 | OTel Feign Instrumentation | 使用 W3C `traceparent` | A-C |
| execution-trace-service | SPLIT + ADAPT | 时间线、调用树、管理 UI | AgentEvent Store/UI + OTel Backend | 技术 Trace 与临床审计分离 | C-F |
| 五步循证流程 | ADAPT | 信息缺口、分层候选、证据回填、回退 | Capability Clinical Policy + Fallback Workflow | 不作为不可变全局步骤 | A-E |
| 三层候选 Tier1/2/3 | ADAPT | 最可能、必须排除、积极备选 | Hypothesis Pack | 场景化、证据化、动态更新 | A-D |
| 终点结论包 | ADAPT | 当前判断、必须排除、依据、行动随访 | DeliveryPackage | 区分患者/医生/系统输出 | D-E |
| 分层错误处理 | KEEP + ADAPT | 工具/服务/系统/业务错误分类 | Runtime Error Policy + Safety Failure Policy | 技术降级与临床降级分离 | A-F |
| Static Case Set | KEEP + EXTEND | 静态病例回归 | Capability Eval Suite | 增加版本快照和安全指标 | A-F |
| Interactive Interview Set | KEEP + EXTEND | 多轮问诊质量与效率 | Conversation Eval | 增加重复、偏航和安全测试 | A-F |
| Trajectory Replay Set | KEEP + EXTEND | 信息逐步揭示后的修正能力 | Replay Eval | 增加 Crash/Resume 和版本回放 | C-F |
| Knowledge Sandbox/Staging/Production | KEEP + ADAPT | 候选区、验证区、生产只读区 | Knowledge Release Lifecycle | 绑定 Source、License、Embedding 和 Eval | A-D |
| Knowledge Publish Gate | KEEP + ADAPT | Schema、来源、回归、回滚门禁 | Knowledge Release Gate | 增加 Citation、适用性和撤回 | A-F |
| Knowledge Evolution Agent 集群 | ARCHIVE + DEFER | 自动抽取、验证、冲突和监控研究 | 后续 Knowledge Ops | V1 先用确定性 Pipeline 和人工审核 | 后续 |
| Neo4j / DR.KNOWS 路径 | EVALUATE + ADAPT | 概念关系、多跳和 Query Expansion | Knowledge Graph Enhancement | 不替代指南、EvidenceClaim 和 Citation | D+ |
| 路径注入 LLM | EVALUATE + ADAPT | 受控上下文、路径外输出验证 | Evidence/Model Runtime | 验证失败不得把路径直接当诊断 | D+ |
| ReactFlow Trace 可视化 | KEEP + ADAPT | 流程、调用树和路径展示 | Clinician/Admin Observability UI | 分离 Trace、AgentEvent、Decision | C-F |

该表为初始设计级结论。Phase A 必须把每一行升级为有代码、测试、运行或数据证据的结论。
...
- Tool 的副作用必须经过 Capability Policy 和幂等控制。
...
| `ExecutionTraceAspect` | ADAPT | OTel Span Adapter | 生成/附着 Span，记录安全属性，不同步调用远程 Trace 服务 |
| `TraceContext` | REWRITE | OpenTelemetry Context + MDC | 使用标准上下文传播；异步使用 TaskDecorator/Reactor Context |
| `FeignTraceInterceptor` | ADAPT | OTel Feign Instrumentation | 注入 W3C `traceparent`/`tracestate`，不在业务 Header 中存内部时钟 |
...
5. ThreadLocal 必须在同步、异步、线程池和响应式场景中有明确传播策略；
6. AOP 自调用、private method 和代理边界必须有测试；
7. 领域 AgentEvent 与技术 Span 必须可通过 run_id/trace_id 关联，但不能混成同一记录。
...
验收：追踪失败不改变业务结果；Trace 连通；无完整 PHI；开销达标；AgentEvent 与技术 Trace 可关联。
...
| 原评估集 | 目标评估集 | 新增内容 |
|---|---|---|
| Static Case Set | Capability Static Case Eval | 红旗、边界、不确定性、版本快照 |
| Interactive Interview Set | Conversation Policy Eval | 信息增益、重复问题、偏航、攻击和停止 |
| Trajectory Replay Set | Durable Replay Eval | Crash/Resume、版本固定、状态一致性 |
...
原评估材料必须先盘点真实文件、样例数量、标注来源、许可和可运行程度，不能只根据文档名称判定为可用。
...
Extractor、Verifier、Conflict Resolver、Release Builder、Shadow Evaluator 等设计作为后续 Knowledge Ops 研究资产保留，但 Medical RAG V1 先使用：
...
以下内容可能保留历史或研究价值，但不得原样进入目标生产主链路：
...
| Clinical Domain Contracts | ToolContext、ToolResult、CDP/AgentState DTO 设计 |
| Clinical State & Data Foundation | CDP 聚合视图、Copy-on-Write、版本历史 |
| Safety & Policy Engine | 红旗、风险、升级、拒答和错误安全原则 |
| Clinical Intelligence | 双通道中的结构化推理、问题清单、信息缺口、三层候选 |
| Evidence Intelligence & RAG | Evidence、知识库优先、路径验证、Knowledge Gate |
| Agent Runtime | Observe/Plan/Act/Update/Evaluate、预算、停止与回退 |
| Context & Memory | AgentState 摘要、允许字段和上下文限制思想 |
| Tool, Skill & Model Governance | 无状态工具、质量、降级、Prompt/Model 适配资产 |
| Durable Execution | AgentState、版本、恢复、工具尝试记录 |
| Observability, Audit & Evaluation | AOP、Trace、AuditTrail、三类评估集、ReactFlow UI |

---

## 15. Phase A：Legacy Design Asset Validation

在现有 Phase A 中增加正式工作包：

```text
A7 Legacy Design Asset Validation
```
...
### 15.3 `legacy-design-asset-inventory.csv` 字段
...
必须增加 Legacy Design 维度，不只盘点服务和代码。每项设计必须关联代码、配置、数据、测试和证据等级。
...
进入 Phase A 后，第一项任务不是机械移动目录，而是证明哪些旧资产真实存在、可运行、可测试，并将有价值内容抽取进 Shared Contracts、`adult_respiratory_v1`、Model Runtime、Knowledge Release、OTel 和 Eval Suite。