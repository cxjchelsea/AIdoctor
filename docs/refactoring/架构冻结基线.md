...
该另立工程基线的范围由 RESCOPE-01 治理。已审查的 V1 快照由 [phase-a-engineering-freeze-record-v1.md](./plans/phase-a/engineering-baseline/phase-a-engineering-freeze-record-v1.md) 记录；现行机器可读判断见 [phase-a-engineering-freeze-effective-gates-v1.csv](./plans/phase-a/engineering-baseline/phase-a-engineering-freeze-effective-gates-v1.csv)。历史门禁登记表保持历史真值，不等于现行冻结判断。该 Engineering Freeze **永不**满足本文原 §12 Frozen Baseline，也不等于 A11 PASS。本记录在独立审查、标准合并与 post-merge verification 成功之前，不把 `PHASE_A_ENGINEERING_BASELINE` 写成已 durable `FROZEN`。

```text
RESCOPE_01_DOES_NOT_REDEFINE_ORIGINAL_PHASE_A_FROZEN_BASELINE
ORIGINAL_PHASE_A_FROZEN_BASELINE: NOT_READY_BLOCKED_CLINICAL
ENGINEERING_FREEZE != ORIGINAL_SECTION_12_FROZEN_BASELINE
ENGINEERING_FREEZE != A11_PASS
```
