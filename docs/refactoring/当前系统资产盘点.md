...
A1 的 diagnosis 编译失败仍是有效的修复前快照。后续独立修复已在 Java 8 下通过干净 compile/test/package，并增加 20 个针对修复点的单元测试，见 [diagnosis-service 构建修复报告](./evidence/phase-a/a1-diagnosis-build-repair/diagnosis-build-repair-report.md)。

因此 `diagnosis-service` 当前可升级为 `BUILD_VERIFIED`；CDP 类型/快照、FieldWriter、EvidenceFusion、Manager/ToolCaller 规范调用和 AgentLoop fail-closed 仅达到局部 `TEST_VERIFIED`。没有服务启动或真实数据证据，仍无 `RUNTIME_VERIFIED`/`DATA_VERIFIED`。AgentLoop 临床规划策略仍为显式 fail-closed blocker。
