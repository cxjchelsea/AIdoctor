...
| StatePatch / Commit / Conflict | `TEST_VERIFIED`（规则边界） | `EVIDENCE_LINKED` | 实现并验证 State Committer、并发、幂等、角色授权 | [覆盖矩阵](./evidence/phase-a/a5/contract-coverage-matrix.csv) |
...
| PatientDeliveryView | `TEST_VERIFIED`（字段白名单） | `PARTIALLY_VALIDATED` | 实现授权 API/UI、医生审核、版本一致性和患者 E2E | [覆盖矩阵](./evidence/phase-a/a5/contract-coverage-matrix.csv) |

这里的 `TEST_VERIFIED` 仅证明 13 个 Schema、13 个正例、26 个反例和 35 个显式规则测试；不证明任何现有服务已迁移，不证明临床正确或 PatientDelivery 已可用。A5 总体保持 `PARTIALLY_VALIDATED`。
