...
| PatientDeliveryView | `TEST_VERIFIED`（结构白名单与 UI 映射） | `PARTIALLY_VALIDATED` | 实现授权 API/UI、自由文本 Policy/Content Safety、医生审核与患者 E2E | [覆盖矩阵](./evidence/phase-a/a5/contract-coverage-matrix.csv) |

这里的 `TEST_VERIFIED` 仅证明 13 个 Schema、13 个正例、33 个结构反例和 107 个显式规则测试；不证明任何现有服务已迁移，不证明临床正确、自由文本内容安全或 PatientDelivery 已可用。A5 总体保持 `PARTIALLY_VALIDATED`。
