...
`DATA_VERIFIED` 仅覆盖明确列出的 tracked CSV/JSON 结构解析；不覆盖 live store、生产数据、临床正确性、运行时摄取或发布批准。

| 能力/旧资产 | A4 Evidence Level | A4 状态 | A5/后续门禁 | 证据 |
|---|---|---|---|---|
| Data Store Inventory | `CODE_CONFIRMED`; 文件子集 `DATA_VERIFIED` | `PARTIALLY_VALIDATED` | 19 项存储需绑定 owner、authority、retention、backup/restore 与安全环境验证 | [存储](./evidence/phase-a/a4/data-store-inventory.csv) |
