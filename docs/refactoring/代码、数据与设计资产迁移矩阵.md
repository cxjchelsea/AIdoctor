...
| Neo4j / Knowledge Graph | 18 labels、22 relations、17 constraints、9 indexes 为代码定义；导入器 TODO 且没有数据证据 | `EVALUATE + ADAPT` 不变；来源、版本、隔离、撤回、对照实验和 Release Gate 前不得生产启用 | [实体](./evidence/phase-a/a4/data-entity-schema-inventory.csv) |
...
| 上传/OCR/日志/Trace | 上传文件与 DB 非原子；Trace/Audit/CDP/日志可重复承载 PHI；删除和保留不完整 | 继续 `SPLIT/ADAPT`；后续分别建立 SourceArtifact、PHI-safe Trace/Audit 与生命周期治理 | [生命周期](./evidence/phase-a/a4/data-lifecycle-governance-inventory.csv) |
| 知识源、派生文件与 DRKnows | 结构性解析覆盖指定 tracked 文件；来源、许可、release、withdrawal 和临床验证不足 | 保持 `EVALUATE/ADAPT/ARCHIVE` 候选；A6.5/D/F 完成 provenance、许可、版本和下线门禁 | [A4 基线](./evidence/phase-a/a4/data-inventory-baseline.md#tracked-data-structural-verification) |

A4 不改变既有迁移类型，不将 live database、Redis、Neo4j、Milvus 或生产数据标为 `DATA_VERIFIED`。
