...
`diagnosis-service` 的独立 Java 8 构建修复证据见 [修复报告](./evidence/phase-a/a1-diagnosis-build-repair/diagnosis-build-repair-report.md)。A1 原始失败仍代表修复前状态；当前 diagnosis 已达到 `BUILD_VERIFIED`，但只对 CDP 类型/快照、FieldWriter、EvidenceFusion、规范 Manager 调用和 AgentLoop fail-closed 产生局部 `TEST_VERIFIED`。

既有 `SPLIT + ADAPT` 决定不变：没有运行或数据证据，没有改变 fixed Workflow、数据库、API、患者输出或迁移决策；AgentLoop 未完成临床策略仍为 blocker。
