...
| Java/Python CDP reader/writer | 创建 StatePatch、CommitResult、ContractConflict；规范化 proposal 路径并闭合状态矩阵 | `WRAP/SPLIT` 保持；A5 不实现单写者或精确字段授权 | [覆盖矩阵](./evidence/phase-a/a5/contract-coverage-matrix.csv) |
...
| 前端诊断结果与患者证据设计 | 创建 PatientDeliveryView 结构白名单、limited/degraded、Source/Conflict/Applicability/Limitation、review/display 和版本绑定 | `REWRITE/ADAPT` 保持；API/UI、自由文本内容安全和医生审核延后 | [覆盖矩阵](./evidence/phase-a/a5/contract-coverage-matrix.csv) |
