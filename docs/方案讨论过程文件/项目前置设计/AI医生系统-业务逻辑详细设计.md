...
> **说明**：本节详细设计知识查询服务和知识运维服务的业务逻辑，包括知识库查询、路径检索验证、知识演化触发、知识提案处理等。

### 5.1 知识查询服务业务逻辑（knowledge-query-service）
...
#### 5.3.3 Conflict Resolver Agent（冲突解决Agent）

**业务逻辑**：
1. **识别冲突**：识别KO之间的冲突类型
2. **选择解决策略**：覆盖/并存/分支/降级
3. **执行解决**：根据策略解决冲突
4. **记录解决信息**：记录冲突解决理由和策略
...
#### 5.3.4 Release Builder Agent（打包发布候选Agent）

**职责**：打包发布候选

**业务逻辑**：
1. **收集通过验证的KO**：从Staging区域收集所有status=verified的KO
2. **生成candidate_release**：
   - 创建候选发布包ID（格式：RELEASE_YYYYMMDD_NNN）
   - 包含所有通过验证的KO列表
   - 生成变更摘要（diff）
   - 生成测试计划
3. **准备发布材料**：
   ...
   - 影响范围评估
   - 风险评估报告
4. **提交Publish Gate**：将candidate_release提交到Publish Gate进行评估

**变更摘要（diff）生成**：
- **对比基线版本**：与当前Production版本对比，识别变更
- **变更类型**：
  - **新增**：新增的KO列表
  - **修改**：修改的KO列表（包含修改前后对比）
  - **删除**：删除的KO列表（标记为deprecated）
- **影响范围评估**：
  - 根据KO的impact_scope评估影响模块（DDx/workup/treatment/risk）
  - 根据downstream_bindings评估影响的组件
  - 评估变更对诊断流程的影响

**测试计划生成**：
- **必跑测试集**：100个固定测试用例（核心DDx场景50个、红旗病种20个、关键路径30个）
- **抽样大集**：500个随机抽样病例
- **测试优先级**：根据变更内容确定测试重点
...
```json
{
  ...
  "impact_assessment": {
    "affected_modules": ["DDx", "workup"],
    "affected_components": ["tool_3", "tool_4"],
    "risk_level": "medium"
  },
  "test_plan": {
    "core_regression": {
...
#### 5.3.5 Shadow Evaluator Agent（影子评测Agent）

**职责**：在离线环境进行评测

**业务逻辑**：
1. **接收candidate_release**：从Release Builder Agent接收候选发布包
2. **准备评测环境**：
   - 加载候选版本的知识库（Staging区域）
   - 加载基线版本的知识库（当前Production版本）
   - 准备测试用例集
3. **执行评测**：
   - **下游诊断性能评测**：
     - 在离线病例集/仿真环境运行诊断流程
   ...
   - **失败模式分布分析**：
     ...
     - 评估失败模式的严重程度
4. **生成评测报告**：
   - 测试集结果统计（core_regression、sampling_set）
   - 指标对比（与基线版本对比）
   - 硬阈值和软阈值检查结果
   - 失败模式分析
   - 评测报告hash
5. **提交评测结果**：将评测报告提交给Publish Gate（Gate-3）
...
- **路径可追溯性**：
  - 路径完整性检查（关键路径断裂数是否=0）
  - 路径可追溯性得分（与基线对比，下降是否≤5%）
  - 证据链完整性得分（与基线对比，下降是否≤3%）
- **失败模式分布**：
  - 失败用例分类（误诊、漏诊、路径断裂等）
  - 失败模式严重程度评估
  - 失败模式分布特征分析
...
准备评测环境
  ├─ 加载候选版本知识库（Staging）
  ├─ 加载基线版本知识库（Production）
  └─ 准备测试用例集（100个固定用例 + 500个抽样用例）
  ↓
执行评测
  ├─ 下游诊断性能评测
  ├─ 路径可追溯性评测
...
生成评测报告
  ...
  └─ 评测报告hash
  ↓
提交评测结果给Publish Gate（Gate-3）
```

#### 5.3.6 Rollback & Drift Monitor Agent（回滚与漂移监控Agent）
...
2. **触发回滚条件检测**：
   ...
   - **人工审核触发条件**：
     ...
     - DDx波动异常但未达到自动回滚阈值
3. **执行回滚**：
   - **全局回滚**：修改current_release指针回退到旧版本
...
1. **Gate-1：结构合法性检查**
   ...
   - concept标准化字段齐全
   - 避免脏数据
   - **通过条件**：所有必需字段存在、字段格式符合规范、数据类型正确

2. **Gate-2：证据可追溯性检查**
   - 每条KO必须绑定provenance
   - 没有证据不允许进入发布候选
   - **通过条件**：所有KO都有provenance、provenance信息完整（doc_id、段落定位、发布日期/版本）
...
**业务逻辑**：
1. **通过Publish Gate后生成新版本**
2. **版本号格式**：v1.0, v2.0, v2.1等
3. **写时复制**：将Staging中的KO复制到Production，标记新版本
4. **更新current_release指针**：指向新版本

#### 5.4.2 版本切换

**业务逻辑**：
1. **修改current_release指针**：指向目标版本
2. **通知相关服务**：通知knowledge-query-service更新版本
3. **记录切换日志**：记录切换原因和操作者

#### 5.4.3 回滚机制

**业务逻辑**：
1. **全局回滚**：修改current_release指针回退到旧版本
2. **局部回滚**：按ko_id局部回滚，利用downstream_bindings进行精确回滚
3. **回滚验证**：验证回滚后的系统状态
...
1. **识别受影响组件**：
   ...
   - 包括：工具（tool_3、tool_4等）、规则引擎、路径模板等
   - 查询每个binding的usage_context和binding_strength
...
3. **回滚执行**：
   ...
   - 设置deprecated_at和deprecated_reason
...
**流程**：
```
检测到问题（Rollback & Drift Monitor Agent）
  ↓
评估回滚范围
  ├─ 全局回滚：修改current_release指针
  └─ 局部回滚：按ko_id局部回滚
      ↓
      识别受影响组件（基于downstream_bindings）
      ↓
      影响范围评估
      ├─ 评估对每个组件的影响
      ├─ 评估是否需要更新配置
...
**业务逻辑**：
1. **灰度方式选择**：
   - **按会话分流**：根据session_id进行分流（如：session_id % 100 < 10，则使用新版本）
   - **按机构分流**：根据机构ID进行分流（如：特定机构使用新版本）
   - **按病种分流**：根据疾病类型进行分流（如：特定疾病使用新版本）
   - **组合分流**：可以组合多种分流方式

2. **灰度策略**：
   - **可借用现有约束字段**：
     - work_mode（工作态）
     - risk_level_limit（风险等级限制）
     - 其他业务约束字段
   - **逐步扩大灰度范围**：
     - 初始灰度：5%流量
     - 逐步扩大：10% → 20% → 50% → 100%
...
```json
{
  "release_id": "v2.1",
  "gray_release_config": {
    "gray_mode": "session_based",
    "gray_ratio": 0.05,
    "gray_rule": "session_id % 100 < 5",
    "monitoring_metrics": [
      "diagnosis_accuracy",
      "red_flag_recall",
      "error_rate"
...
**文档版本**：v4.0（基于DR.KNOWS的单主Agent + 多工具Tools架构 + 知识演化与维护）  
**创建日期**：2025年1月  
**更新日期**：2025年1月  
**文档定位**：AI医生系统的业务逻辑详细设计（工具的工作流程、DR.KNOWS核心方法、CDP管理、知识查询与知识演化等）  
**参考文档**：《AI医生系统-系统功能设计.md》、《AI医生系统-技术架构设计.md》、《知识演化与知识维护-完整设计方案.md》  
**设计基础**：基于DR.KNOWS论文，采用单主Agent + 多工具Tools架构设计，集成知识演化与维护系统  
**更新说明**：根据DR.KNOWS设计，更新为工具的工作流程，添加DR.KNOWS核心方法（知识图谱路径检索、路径评分排序、路径注入LLM），添加CDP管理业务逻辑，整合健康管理态和临床诊疗态两种工作态，新增知识查询与知识演化业务逻辑。
