...
> **文档定位**：本文档是AI医生系统的**技术架构设计**的CDP数据与状态管理部分，包含CDP数据结构设计、AgentState设计、AuditTrail设计、API设计、证据清单存储、回填与重排规则、回退机制、复评与升级规则、终点结论包生成，以及CDP流转与状态管理。  
> **业务功能**：请参考《AI医生系统-系统功能设计.md》  
> **核心目标**：详细说明CDP数据模型、AgentState数据模型、AuditTrail数据模型、版本管理、状态转换和持久化策略。
...
#### tool_call 字段（event_type=tool_call时）
...
#### cdp_update 字段（event_type=cdp_update时）
...
#### agent_decision 字段（event_type=agent_decision时）
...
# AgentState管理API
POST   /api/v1/agent_state             # 创建AgentState
GET    /api/v1/agent_state/{session_id} # 获取AgentState
PUT    /api/v1/agent_state/{session_id} # 更新AgentState
...
CDP更新（健康管理）    CDP更新（信息收集）
    - 【主Agent】调用病例理解工具（tool_1）更新 patient_state
    - 生成 wellness_plan
    - 持续监控          - 【主Agent】调用病例理解工具（tool_1）更新 patient_state
         │              - 【主Agent】调用主动问诊工具（tool_2）生成问诊计划
         │                   ↓
         │              CDP更新（诊断阶段）
         │              - 【主Agent】调用鉴别诊断工具（tool_3）更新 ddx
         │              - 【主Agent】调用检查建议工具（tool_4）更新 workup_plan
         │              - 【主Agent】调用风险评估工具（tool_6）更新 triage
         │                   ↓
         │              CDP更新（处置阶段）
         │              - 【主Agent】调用治疗建议工具（tool_5）更新 management_plan
         │              - 【主Agent】调用证据链工具（tool_7）更新 evidence_graph
         │                   ↓
         │              CDP持久化（诊断完成）
         │                   ↓
         │              CDP更新（随访阶段）
...
1. **读写锁机制**：
   - 多个工具可以同时读取CDP（通过主Agent）
   - 写入CDP需要获取写锁（主Agent统一管理）
   - 写锁按工具优先级分配

2. **冲突检测**：
   - 检测多个工具同时写入同一字段（主Agent内部检测）
   - 使用版本号检测并发冲突
   - 冲突时主Agent进行evidence fusion和conflict resolution
...
主Agent维护以下策略状态（详见6.2节AgentState数据结构设计）：
...
- AgentState存储在数据库中（详见6.2节）
- 每次主Agent决策时更新AgentState
- 支持状态查询和恢复
