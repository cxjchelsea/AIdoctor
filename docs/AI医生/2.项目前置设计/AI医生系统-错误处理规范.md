...
5. **安全考虑**：不暴露系统内部信息给用户
6. **可追溯性**：错误必须可关联到CDP、智能体、请求链路（便于审计/回放/复盘）
...
| `agentId` | 否 | 产生错误的智能体/脑区（如 `agent_3` / `orchestrator`） |
| `cdpId` | 否 | 关联CDP ID（涉及CDP读写必须带） |
| `retryable` | 否 | 是否建议客户端/编排器自动重试（true/false） |
| `timestamp` | 是 | 毫秒时间戳 |
| `path` | 是 | 请求路径 |
...
#### diagnosis-service（协调器/编排器）错误码（2000-2099）
...
| 2006 | 流程已取消 | 400 |
| 2007 | 下游服务调用失败（非超时） | 502 |
| 2008 | 下游服务调用超时 | 504 |
| 2009 | 下游服务不可用（熔断/健康检查失败） | 503 |
| 2010 | 协商/冲突解决失败（多智能体分歧未收敛） | 500 |
| 2011 | 消息队列投递失败（异步任务） | 503 |
| 2012 | CDP读写失败（封装/落库/缓存） | 500 |
| 2013 | 诊断记录不存在 | 404 |
...
- **CDP更新失败**：优先返回 `6001/1702` 并提示“请重试”；编排器可按幂等键重放。
...
**参考文档**：《AI医生系统-系统功能设计.md》、《AI医生系统-技术架构设计.md》  
**设计基础**：基于DR.KNOWS论文的八个脑区架构设计  
**更新说明**：修正错误码分段冲突（examination-service改为2100段），补齐编排器/并发/调用治理错误码；统一错误响应契约字段（traceId/cdpId/agentId/retryable）；对齐跨服务调用超时重试与降级策略；修正Python业务异常不再统一返回HTTP 200。
