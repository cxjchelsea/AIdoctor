...
> 状态：`M2_REVISION_APPLIED / RE_REVIEW_REQUIRED / POLICY_PAIR_FREEZE_BLOCKED / D_STILL_BLOCKED`。  
...
|---|---|---|---|
| M2-R1 | Required Evidence 是否显式包含 UNMEASURED → INPUT_INSUFFICIENT | PENDING | PENDING |
| M2-R2 | RR/SBP/HR 的 NOT_ASKED / AMBIGUOUS / CONFLICTING 是否显式 → INPUT_INSUFFICIENT | PENDING | PENDING |
| M2-R3 | INPUT_INSUFFICIENT 是否显式携带 RULE_SIGNAL_INPUT_INSUFFICIENT | PENDING | PENDING |
| M2-R4 | 修订是否未改变 15 条 active rule predicate / threshold | PENDING | PENDING |
| M2-R5 | 修订是否未改变 Shared Scope、未开始 Eval、未引入 D09 disposition | PENDING | PENDING |
...
才允许下一步把两个 policy **一起**进入 freeze decision。
...
Missingness M2 Re-review = REQUIRED
Missingness Policy Frozen = NO
Shared Scope Policy Frozen = NO / HELD
