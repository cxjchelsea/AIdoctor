...
C v0.2 条目级已批准，package 仍有 BF-C-04。下一动作是按 `U03_C_Revision_Task_v0.2.md` 关闭该 blocker；D 仍须等 package 再批准与 freeze。
...
C = STRUCTURAL_SCHEMA_FROZEN / DRAFT_v0.2_REVIEWED / RULE_LEVEL_APPROVED / PACKAGE_REVISE_BF-C-04
D = STRUCTURAL_SCHEMA_FROZEN / CLINICAL_CONTENT_NOT_STARTED / BLOCKED_UNTIL_C_PACKAGE_AND_FREEZE
...
当前唯一合理的下一步：按 `U03_C_Revision_Task_v0.2.md` 关闭 BF-C-04。D 不得先于 C 的 package 再批准 / freeze 开始。