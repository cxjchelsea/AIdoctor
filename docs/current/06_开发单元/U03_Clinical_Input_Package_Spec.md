...
```text
STRUCTURAL_SCHEMA_FROZEN
CLINICAL_POLICY_CONTENT_DRAFT_v0.2 = REVIEWED
MEDICAL_PRIORITY_PRECEDENCE = APPROVED_P0_TO_P5
MEDICAL_OWNER_REVIEW = COMPLETE
TECHNICAL_REVIEW = COMPLETE
CONTENT_APPROVAL = APPROVED_FOR_CONTENT_AND_COVERAGE
POLICY_RELEASE = PR-U03-D09-001@0.2.0-draft
COVERAGE_CONTRACT = U03_D09_COVERAGE_V0_2 / NOT_FROZEN
CD-05 = NOT_PASSED
```

D v0.2 内容与 coverage contract 已再审通过；BF-D-01 / BF-D-02 已关闭。这不等于 candidate freeze、CD-05 或 Gate B。
...
```text
STRUCTURAL_SCHEMA_FROZEN
GOLDEN_CASE_CONTENT = 31_APPROVED_FOR_EVALUATION_CONTENT
SAFETY_SUITE_CONTENT = 20_CANDIDATES_CONTENT_APPROVED
MEDICAL_OWNER_REVIEW = COMPLETE / APPROVE
EVALUATION_OWNER_REVIEW = COMPLETE / APPROVE
CD-06 = REVIEW_READY
Gate C = NOT_PASSED
```
